...
echo "   2. Validate the config:    rustydnsd --config $INSTALLED_CONFIG --validate-config"
echo "   3. Enable and start:       systemctl enable --now rustydns"
echo "   4. Check status:           systemctl status rustydns"
echo "   5. Follow logs:            journalctl -u rustydns -f"
echo ""
echo " Verify it's working (after start):"
echo "   curl -s http://127.0.0.1:9153/health | jq"
echo "   dig @127.0.0.1 example.com +short          # should resolve"
echo "   dig @127.0.0.1 doubleclick.net +short      # should return NXDOMAIN"
echo "   curl -s http://127.0.0.1:9153/metrics | grep blocklist_hits_total"
echo ""
...
echo ""
echo " Full README + Troubleshooting:"
echo "   https://github.com/Iwan-Teague/rustydns#readme"
