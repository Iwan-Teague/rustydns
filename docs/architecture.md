...
| Oblivious DoH (target never learns client IP) | RFC 9230 | ✓ implemented — `upstream.protocol = "odoh"` + `upstream.odoh_proxy`; HPKE via `odoh-rs`, relayed through the oblivious proxy. Fail-closed (never falls back to plain DoH); no *client-side* DNSSEC on this arm (`dnssec_validation` must be `false`). See `docs/roadmap.md` §ODoH and `crates/rustydns-resolver/src/odoh.rs` |
