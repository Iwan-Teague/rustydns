...
- Signal handling: `SIGHUP` re-reads blocklist content, the mesh-zone bundle, and `rustydns.toml`. Config reload hot-swaps the upstream resolver, per-client policy, and rate limiter atomically via `ArcSwap` (no dropped in-flight queries). Listener addresses, DoT/DoH/TLS material, the metrics binding, blocklist *sources*, and the on-disk query log are bound/spawned at startup and still require a restart — such changes are logged on reload but not applied. A config that fails to parse/validate leaves the running config untouched. `SIGTERM`/`SIGINT` runs the bounded graceful shutdown (`RUSTYDNS_SHUTDOWN_TIMEOUT_SECS`, default 10s); a second signal collapses the timeout to zero.
