...
#[tokio::test(flavor = "current_thread")]
async fn binary_e2e_txt_record_served_from_authority() {
    // TXT RECORDS: SPF data stored as an authority static record must be
    // served correctly over UDP alongside A records for other names.
    let (dns_port, upstream_port, metrics_port) = pick_ports();
    let tmp = tempfile::TempDir::new().expect("tempdir");
    ...
    // Control: A query for router.mesh. still works.
    let a_reply = resolve_a(&sock, 98, "router.mesh.").await;
    assert_eq!(a_reply.metadata.response_code, ResponseCode::NoError);
    assert_eq!(a_reply.answers.len(), 1);
