...
```
 client query (UDP/TCP/DoT/DoQ/DoH)
         │
         ▼
  ┌─────────────┐
...
- **Response-IP denylist** — `blocklist.response_ip_denylist` is a list of IP/CIDR ranges (IPv4/IPv6); a query whose resolved A/AAAA rdata falls inside any range is blocked post-resolution (per `block_response`). Complements the rebinding defence (which strips *private* rdata) by letting operators name *arbitrary* bad ranges (malware C2, ad-network IPs). Dependency-free CIDR matcher in `rustydns_core::ip_denylist`.
- **DNS rewrites (`[[rewrite]]`)** and **Safe Search (`[safesearch]`)** — operator-defined local overrides served at the rewrite stage (after authority, before blocklist). Safe Search is sugar over rewrites: enabling it injects built-in CNAME rules mapping each search engine to its SafeSearch/Restricted-Mode hostname (`google.com → forcesafesearch.google.com`, etc.), merged with operator `[[rewrite]]` rules (explicit rules win). Both hot-reload on SIGHUP.
...
- DoQ listener (optional, RFC 9250): DNS-over-QUIC on a UDP socket, gated on the same `tls_cert_path`/`tls_key_path` (one certificate serves both DoT and DoQ). The TLS config carries the `doq` ALPN. Rejected by `validate_config` if the cert/key are missing.
