...
- **Per-source-IP token-bucket rate limiting.** Default-on with a generous 100 qps
  ...
  eviction + periodic GC of idle buckets so a forge-source-IP flood cannot OOM the
   ...
   IPv4-mapped IPv6 addresses (`::ffff:a.b.c.d`, reported by dual-stack sockets
   for v4 peers) are unwrapped first, so each v4 client keys on its own `/32`
   and mapped loopback still gets the exemption.
